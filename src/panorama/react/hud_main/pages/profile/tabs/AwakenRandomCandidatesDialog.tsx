...
const SLOTS = [0, 1, 2];
// 每次轮播的间隔（秒），递增实现减速；跑完后若候选未到则按 SLOW 继续慢闪直到到达
const ROLL_DELAYS = [0.05, 0.05, 0.06, 0.07, 0.09, 0.11, 0.14, 0.18, 0.23, 0.3];
const ROLL_SLOW_DELAY = 0.12;
 ...
 * 轮播天然盖住抽选请求的网络延迟；选 1 个进入确认，关闭/退出免费、无副作用。
 */
        ...
        <Panel className="awaken-candidates-row">
          {SLOTS.map((i) => {
            ...
            return (
              <Panel
                key={i}
                className={settled ? 'awaken-card awaken-card-settled' : 'awaken-card'}
              >
                <DOTAHeroImage
