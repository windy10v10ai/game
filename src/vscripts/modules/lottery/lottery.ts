...
// 新增：每个玩家的被动技能选择计数器
  ...
  constructor() {
    ...
    // 监听技能重选物品使用
    ListenToGameEvent('dota_item_used', (event) => {
    ...
    // 注册技能重选事件 - 显式指定泛型类型
    CustomGameEventManager.RegisterListener<LotteryPickEventData>('skill_reset_pick', (userId, event) => {
      this.pickSkillReset(userId, event);
    });
      // 注册技能重选移除事件
  CustomGameEventManager.RegisterListener<LotteryPickEventData>('skill_reset_remove', (userId, event) => {
  ...
  randomAbilityForPlayer(playerId: PlayerID, isActive: boolean) {
    ...
    if (!isActive && abilityLotteryResults.length > 0) {
      ...
      const randomIndex = RandomInt(0, 5);
        ...
        if(randomIndex==1)
          abilityLotteryResults[0] = { name: 'dazzle_good_juju', level: 5 }; // level可调整
        if(randomIndex==2)
        abilityLotteryResults[0] = { name: 'leshrac_defilement2', level: 4 }; // level可调整
        if(randomIndex==3)
        abilityLotteryResults[0] = { name: 'death_prophet_witchcraft2', level: 3 }; // level可调整
        if(randomIndex==4)
        abilityLotteryResults[0] = { name: 'medusa_split_shot', level: 5 }; // level可调整
        if(randomIndex==5)
    ...
    if (isActive && abilityLotteryResults.length > 0) {
      ...
      if(randomIndex==0)
        abilityLotteryResults[0] = { name: 'marci_unleash', level: 3 }; // level可调整
      if(randomIndex==1)
        abilityLotteryResults[0] = { name: 'dazzle_bad_juju', level: 3 }; // level可调整
      if(randomIndex==2)
      ...
      if(randomIndex==5)
        abilityLotteryResults[0] = { name: 'tinker_rearm_lua', level: 3 }; // level可调整
      if(randomIndex==6)
        abilityLotteryResults[0] = { name: 'juggernaut_omni_slash', level: 3 }; // level可调整
  }
    // 修改修改结束
...
if (activeEffectPassiveAbilities.length > 0) {
  print('[Lottery] 有主动效果的被动技能列表: ' + activeEffectPassiveAbilities.map(a => a.name).join(', '));
}
...
if (selectedAbilities.length < 2) {
  ...
  print('[Lottery] 优先级2(有主动效果的被动)后,已选择: ' + selectedAbilities.length + ' 个 (新增 ' + (selectedAbilities.length - beforeCount) + ' 个)');
}
...
if (selectedAbilities.length < 2) {
  ...
  print('[Lottery] 优先级3(纯被动,从后往前)后,已选择: ' + selectedAbilities.length + ' 个 (新增 ' + (selectedAbilities.length - beforeCount) + ' 个)');
}
