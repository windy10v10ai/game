...
function goku_kaioken:OnSpellStart()
    ...
    self.particle_model = ParticleManager:CreateParticle("particles/custom/goku/goku_kaioken_active_model.vpcf",
    ...
    local particle_ring = ParticleManager:CreateParticle("particles/custom/goku/goku_kaioken_active_cast_ring.vpcf",
        PATTACH_WORLDORIGIN, caster)
    ...
    for _, enemy in ipairs(self.enemies) do
        local particle_tag = ParticleManager:CreateParticle(
            "particles/custom/goku/goku_kaioken_active_targetted_marker.vpcf", PATTACH_OVERHEAD_FOLLOW, enemy)
    ...
    caster:SetContextThink("AttackEveryOne", function()
        if #self.enemies == 0 then
            if self.target then
                local particle_line = ParticleManager:CreateParticle(
                    "particles/custom/goku/children/goku_kaioken_active_trail.vpcf", PATTACH_WORLDORIGIN, self.target)
        ...
        local particle_line = ParticleManager:CreateParticle("particles/custom/goku/goku_kaioken_active_trail.vpcf",
            PATTACH_WORLDORIGIN, self.target)
        ParticleManager:SetParticleControl(particle_line, 0, self.target:GetAbsOrigin())
        ...
        local particle_hit = ParticleManager:CreateParticle(
            "particles/econ/items/drow/drow_ti6_gold/drow_ti6_silence_gold_arrow_hit.vpcf", PATTACH_WORLDORIGIN, target)
        ParticleManager:SetParticleControlEnt(particle_hit, 1, target, PATTACH_POINT_FOLLOW, "attach_hitloc",
        ...
        if not target:IsBuilding() and target:GetClassname() ~= "dota_item_drop" and target:IsAlive() then
            ...
            if RollPseudoRandomPercentage(chance, DOTA_PSEUDO_RANDOM_CUSTOM_GAME_1, parent) then
                ...
                target:AddNewModifier(parent, ability, "modifier_stunned",
                    { duration = stun_time * (1 - target:GetStatusResistance()), target = target })
                parent:AddNewModifier(parent, ability, "modifier_goku_kaioken_trigger",
                    { duration = 0.3, target_index = target:GetEntityIndex() })
            end
        end
    end
end
...
function modifier_goku_kaioken_trigger:OnCreated(keys)
    ...
    if parent:HasModifier("modifier_goku_super_saiyan") then
        particle = ParticleManager:CreateParticle("particles/custom/goku/goku_kaioken_trigger_super_saiyan.vpcf",
            PATTACH_ABSORIGIN_FOLLOW, self.target)
        ParticleManager:SetParticleControl(particle, 60, Vector(255, 255, 0))
    else
        particle = ParticleManager:CreateParticle("particles/custom/goku/goku_kaioken_trigger.vpcf",
            PATTACH_ABSORIGIN_FOLLOW, self.target)
        ParticleManager:SetParticleControl(particle, 60, Vector(0, 255, 255))
    end
    ParticleManager:SetParticleControlEnt(particle, 1, self.target, PATTACH_POINT_FOLLOW, "attach_hitloc",
        self.target:GetAbsOrigin(), true)
    ParticleManager:SetParticleControlEnt(particle, 2, parent, PATTACH_POINT_FOLLOW, "attach_hitloc",
        parent:GetAbsOrigin(), true)
    ParticleManager:SetParticleControlEnt(particle, 4, self.target, PATTACH_ABSORIGIN_FOLLOW, "",
        self.target:GetAbsOrigin(), true)
