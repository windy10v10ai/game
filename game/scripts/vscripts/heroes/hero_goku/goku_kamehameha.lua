...
function goku_kamehameha:OnAbilityPhaseStart()
	...
	self.particle_storager = ParticleManager:CreateParticle(
		"particles/custom/goku/goku_kamehameha_lase_energy_storager.vpcf", PATTACH_CUSTOMORIGIN_FOLLOW, caster)
	ParticleManager:SetParticleControlEnt(self.particle_storager, 0, caster, PATTACH_POINT_FOLLOW, "attach_attack1",
		caster:GetAbsOrigin(), true)
	ParticleManager:SetParticleControlEnt(self.particle_storager, 1, caster, PATTACH_POINT_FOLLOW, "attach_attack1",
		caster:GetAbsOrigin(), true)
...
function goku_kamehameha:OnSpellStart()
	...
	self.particle = ParticleManager:CreateParticle("particles/custom/goku/goku_kamehameha_laser.vpcf",
		PATTACH_CUSTOMORIGIN_FOLLOW, caster)
	ParticleManager:SetParticleControlEnt(self.particle, 0, caster, PATTACH_POINT_FOLLOW, "attach_attack1",
		caster:GetAbsOrigin(), true)
	ParticleManager:SetParticleControlEnt(self.particle, 9, caster, PATTACH_POINT_FOLLOW, "attach_attack1",
		caster:GetAbsOrigin(), true)
	if caster:HasModifier("modifier_goku_super_saiyan") then
	...
	for i = 1, length / self.width, 1 do
		table.insert(self.FowViwers,
			AddFOWViewer(self:GetCaster():GetTeamNumber(), origin + vector * i * self.width, self.width, 99999, false))
	end
...
function goku_kamehameha:IntervalDetection()
	...
	if self.times ~= t_times then
		...
		local units = FindUnitsInLine(caster:GetTeamNumber(), self.startPos, self.endPos, nil, self.width,
			DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, target_flag)
