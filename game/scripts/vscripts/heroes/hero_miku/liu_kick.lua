...
function modifier_liu_kick:CheckState()
    ...
    if self.target and not self.target:IsNull() and
        self.target:HasFlyMovementCapability() then
...
function modifier_liu_kick:OnCreated(hTable)
    ...
    if IsServer() then
        ...
        self.range_knockback =
            self.ability:GetSpecialValueFor("range_knockback")
...
function modifier_liu_kick:UpdateHorizontalMotion(me, dt)
    ...
    if (self.target:GetOrigin() - self.parent:GetOrigin()):Length2D() <
        self.latch_offset then
        self:PlayEffects()
