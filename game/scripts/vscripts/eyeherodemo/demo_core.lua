...
function HeroDemo:IsDebugPanelEnabled()
    -- 移植到正式地图后只允许在 Tools 调试模式启用，避免线上环境暴露调试面板。
    return IsInToolsMode ~= nil and IsInToolsMode()
end

require("eyeherodemo.demo_events")

if not HeroDemo:IsDebugPanelEnabled() then
    -- 非调试模式只加载事件文件中的全局类定义，不继续注册按钮事件和调试功能。
    return
end
...
function HeroMaxLevel(hHero)
    if not IsNotNull(hHero) then
        return
    end
...
LinkLuaModifier( "lm_take_no_damage", "eyeherodemo/demo_core.lua", LUA_MODIFIER_MOTION_NONE )
...
function HeroDemo:Init()
    ...
    -- Panorama 通过 FireCustomGameEvent_Str 触发这些事件；这里统一绑定到 HeroDemo 的 Lua 处理函数。
    ...
    -- 生成指定阵营英雄是 AIB 面板新增能力，回调内会重新绑定控制权并跳过本地图 bot AI。
...
HeroDemo.OnSpawnToTeamButtonPressed = function(self, iEventIndex, hEvent)
	...
	if IsNotNull(hPlayerHero) then
		...
		-- DebugCreateUnit 会按目标阵营创建真实英雄；创建完成后在回调里修正位置、归属和 AI 状态。
		DebugCreateUnit( hPlayer, sHeroToSpawn, iToTeam, false,
			function(hHeroSpawned)
				-- 标记调试面板创建的英雄，供本地图的出生点和 bot AI 初始化逻辑识别并跳过。
				_G.HeroDemoDebugSpawnedHeroes = _G.HeroDemoDebugSpawnedHeroes or {}
				_G.HeroDemoDebugSpawnedHeroes[hHeroSpawned:entindex()] = true
				hHeroSpawned.bInitialized = true

				FindClearSpaceForUnit(hHeroSpawned, vSpawnLoc, false)

				hHeroSpawned:SetRespawnPosition(vSpawnLoc)
				-- 重新绑定到按下按钮的玩家；SetControllableByPlayer 只授予控制权，不会切断原 PlayerID/Owner。
				hHeroSpawned:SetOwner(hPlayer)
				hHeroSpawned:SetPlayerID(iPlayerID)
				hHeroSpawned:SetControllableByPlayer(iPlayerID, false)
				hHeroSpawned:RemoveModifierByName("modifier_rooted")
				-- 清理回调触发前可能已经挂上的 AI modifier，避免 bot 继续下达命令。
				if GameRules.AI ~= nil and GameRules.AI.getModifierName ~= nil then
					local sAIModifierName = GameRules.AI:getModifierName(hHeroSpawned)
					if sAIModifierName ~= nil then
						hHeroSpawned:RemoveModifierByName(sAIModifierName)
					end
				end
				hHeroSpawned:RemoveModifierByName("modifier_bot_think_strategy")
				hHeroSpawned:RemoveModifierByName("modifier_bot_think_item_use")
				hHeroSpawned:RemoveModifierByName("modifier_bot_think_ward")
				
				-- 关闭自动索敌并立刻 Hold，保证生成后保持静止等待玩家手动操作。
...
function HeroDemo:OnRemoveHeroButtonPressed( eventSourceIndex, data )
	...
	if ( hHero ~= nil and hHero:IsNull() == false and hHero ~= PlayerResource:GetSelectedHeroEntity( nPlayerID ) ) then
