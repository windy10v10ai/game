...
function modifier_bloodseeker_blood_mist2:OnCreated(keys)
	...
	self.particle =  ParticleManager:CreateParticle("particles/units/heroes/hero_bloodseeker/bloodseeker_scepter_blood_mist_aoe.vpcf",1,self:GetCaster())
...
function  modifier_bloodseeker_blood_mist2:OnIntervalThink()
	...
	local damageInfo ={victim = self:GetCaster(),attacker = self:GetCaster(),damage = damage,damage_type = DAMAGE_TYPE_MAGICAL,ability = self:GetAbility(),damage_flags=1024+4096}
	ApplyDamage( damageInfo )
	--搜寻单位
	local caster = self:GetCaster()
	local units = FindUnitsInRadius(caster:GetTeamNumber(),caster:GetOrigin(),nil,450,DOTA_UNIT_TARGET_TEAM_ENEMY,DOTA_UNIT_TARGET_BASIC+1,48,FIND_ANY_ORDER,false)
	for k,v in pairs(units)do
		--造成伤害
		local damageInfo ={victim = v,attacker = caster,damage = damage,damage_type = DAMAGE_TYPE_MAGICAL,ability = self:GetAbility(),damage_flags=1024}
		ApplyDamage( damageInfo )
	end
end
