...
function  modifier_dark_seer_normal_punch2:OnAttackLanded(keys)
	if IsServer() and keys.attacker == self:GetParent() and self:GetAbility():IsCooldownReady() then
		...
		--造成伤害
		local damage = self:GetAbility():GetSpecialValueFor("max_damage")+self:GetCaster():GetAverageTrueAttackDamage(nil)*self:GetAbility():GetSpecialValueFor("ad")
		local damageInfo ={victim = keys.target,attacker = self:GetCaster(),damage = damage,damage_type = DAMAGE_TYPE_MAGICAL,ability = self:GetAbility()}
		ApplyDamage( damageInfo )
		
		keys.target:AddNewModifier(self:GetCaster(),self:GetAbility(),"modifier_stunned",{duration=1.25})
		
		--特效
		local Particle =  ParticleManager:CreateParticle("particles/units/heroes/hero_dark_seer/dark_seer_attack_normal_punch.vpcf",0,self:GetCaster())
