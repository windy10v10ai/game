...
description: >-
  ...
  评论中提炼。用于 GitHub Actions 自动 PR review（.github/workflows/claude-pr-review.yml）
  读取，也可在用户说"review 一下这个 PR"「检查一下这份改动」「这个 PR 有没有问题」
  「按我们的标准看看这段代码」等场景手动触发。
---
...
- 语言跟随 PR 标题/描述判断（忽略模板里的 Release Note 段落，那部分固定中英双语，不能作为判断依据）：PR 以中文为主 → 全篇中文；以其他语言（如英文）为主 → 主体用该语言撰写，末尾附一份中文对照，用分隔线隔开，方便仓库维护者审阅
