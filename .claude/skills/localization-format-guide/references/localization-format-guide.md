...
- **俄文 (`addon_russian.txt`)**：UI 相关键（Panorama 界面文本）必须维护；技能/物品/游戏逻辑类键在**本次新增或修改该键时一并写俄文**，存量中原本缺俄文的旧键不必特意补齐；已存在的俄文一律保留并跟随中英同步更新，不得删除

## 添加新的本地化键

1. 添加到中文文件（`addon_schinese.txt`）
2. 添加到英文文件（`addon_english.txt`）
3. 同步添加到俄文文件（`addon_russian.txt`）——UI 键必须；技能/物品/游戏逻辑类键在本次新增或修改时也一并写
4. 不主动为存量中缺俄文的旧键补齐；已存在的俄文必须与中英同步修改或同步删除
