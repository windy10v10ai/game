...
- 技能/物品/游戏逻辑类键不翻译俄文
- **文案不使用分号（`；`/`;`）**：句子之间用逗号或句号分隔，分号在 UI 短文案里显得生硬
- **`_Description` 不重复单独成行的数值定义**：技能/物品若已为某数值提供单独条目（如 `_shots`、`_duration` 等带冒号的数值标签行），`_Description` 正文只做文字描述、**不再写 `%xxx%` 占位符**复述该数值（游戏会在下方单独成行展示）。仅当该数值**没有**单独定义时，才在 Description 里用 `%xxx%` 内联
