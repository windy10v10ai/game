...
- 百分比数值（仅当最大值 > 50% 时）：如 slow_attack_speed_pct、slow_movement_speed、状态抗性、魔法抗性，最大生命值等。burn_damage 命名看不出来，但 abilities_schinese 描述说明中是百分比
- radius 等固定范围属性

### 示例

```
特殊情况 - 小技能 (最大等级 CD ≤ 10s):
官方 4 级: 8 7 6 5
...
属性名中文描述在 `docs/reference/{version}/abilities_schinese.txt` 中查找
