...
description: >-
  ...
  仅写差分；先删与参考逐字相同的键；**本图 MaxLevel 高于参考默认档数时**，须扫描参考 `AbilityValues` 中**已有多档的 `value`**，在确有延伸规律时补到与有效 `MaxLevel` 同档数（非单档键勿强行拉档）；行尾 `x2`/差值等**即换算规则**，换参考后须按规则重算数值；禁 facet（7.41+）；细则见正文。
---
...
3. **再补差分与档数**：保留的键中，多档 `value` 的档数须与**有效 `MaxLevel`** 一致；参考里**已是多档且随等级变化**的子键，若本图提高了该技能 `MaxLevel`，通常须在 override **写出延伸档**（见下「多档子键与 MaxLevel」）。
4. **最后自检**：无同值抄写、无 facet、保留的多档长度对齐 `MaxLevel`。
...
2. **再**处理确有本图数值差分的键：固定加、倍数、延伸**且**新档相对官方有**实质变化**等；并对照**「多档子键与 MaxLevel」**检查参考里是否还有**未写入 override**、但随技能等级变化且本图已提高 `MaxLevel` 的多档子键。
...
3. **逐键对照参考**：对每个技能，只把**与参考不同**的键写入（或保留）override；**删除**与参考逐字相同的键（含 `AbilityBehavior` 等）。**同一轮内先删「同值 / 同值多档」整键**，再为**确有差分**的键补全与 `MaxLevel` 对齐的档数，并**扫一遍参考 `AbilityValues` 中多档子键**（见「多档子键与 MaxLevel」）避免漏延伸。
4. **分类**：无加强 / 固定差 / 倍数等。
