...
| 参考无、且无上述注释 | 删除 | — |
| CD 末档封顶 | 按差值延伸后末档值 ≤10s（小招）/ ≤60s（大招）时，**保持等差数列**：缩小差值（通常同时略微降低首档），使末档恰好等于封顶值（10s 或 60s）。小招示例：参考 `20 15 10`，MaxLevel=4，调整为 `16 14 12 10`（差值 -2）。大招示例：参考 `90 75 60`，MaxLevel=4，调整为 `90 80 70 60`（差值 -10）。 | 无 |
...
- `question` 用中文描述：`「{中文属性名}（{系统键名}）」：当前官方值 {参考值}，现有 override 值 {当前值}，原注释 {注释}。如何处理？`
