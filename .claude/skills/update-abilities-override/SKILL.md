...
- 行尾有 `x2`、`+N` 等标记 → 按该规则在**当前参考值**上重算
- **无行尾注释** → 默认「同步官方 + 仅等级扩展」，无本图加强；补加注释时须**同步修改 value**，不得只改注释
- **任意键无注释** → 默认仅等级扩展，数值必须与参考相同前几档 + 步长延伸；若与参考不同须加注释说明，否则视为遗漏，应回退为官方值
...
| 情形 | value 处理 | 行尾注释 |
|------|-----------|---------|
| 仅等级扩展 | 前几档同参考，新档按步长延伸 | 无（不写注释） |
| 固定差值 | 每档 = 参考 + N | `// 参考各档 +N` |
| 倍数 | 先推参考第 N 档再乘倍率 | `// 参考各档 xN`（基数**必须**写出） |
| CD 末档封顶 | 小招末 ≤10s / 大招末 ≤60s 时末档取参考末档 | |
| CD 档数不足 | 保持同一档差递减至参考末档，如参考 `12 10 8 6` → `10 9 8 7 6` | |

> **换版本后重算**：先更新 `//` 后的参考数为当前版本值，再按同一规则重算 `value`。若 value 与规则不符，要么改 value 要么换规则描述，禁止二者脱节。
...
| 改步长 | `"3 3.5 4 4.5 5" // 3 4 5 6 差值0.5` |
