...
- 行尾注释**没有**该说明（如仅写 `// x2.5` 或无注释）的处理：
  ...
  - **原版结构变化导致的失效键**（如 `special_bonus_shard`/`special_bonus_scepter`/普通 `AbilityValues` 子键在参考中已不存在）→ **直接删除，无需询问**
...
| 参考无、且无上述注释 | 删除 | — |
| CD 末档 | **封顶值**：小招 10s / 大招 60s；参考档位中的最小冷却值 `min`（块顶 `AbilityCooldown` 多档与 `AbilityValues` 内冷却类 `value` 多档一并取 min；仅单档则该数即 min）。<br>**调整方式**：**优先保持等差数列**，末档锁为 `max(min, 封顶值)`（即若 `min` 本就低于封顶值，则末档=`min`；否则末档=封顶值）。步骤：(1) 尝试保持参考首档不变，新公差 = `(末档 - 参考首档) / (MaxLevel - 1)`；(2) 若无法整除，**微调首档**使公差为整数，末档必须保持不变。<br>**情形1 · 首档不变即整数公差**：参考 `22 18 14 10`（4 档，min=10），MaxLevel=5，等差延伸末档 6 < 10 → 末档锁 10，(10-22)/4=-3 整除 → `22 19 16 13 10`。<br>**情形2 · 调首档以保证整数公差**：参考 `20 17 14 11`（4 档，min=11，公差 -3），MaxLevel=5，延伸第5档=8 < 10 → 触发封顶，末档锁 10。保持首档 20 时 (10-20)/4=-2.5 非整数 → 首档下调至 18，公差 -2 → `18 16 14 12 10`。 | 无 |
