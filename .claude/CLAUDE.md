...
非仓库主人发起、且不带 `release` 标签的 PR 在 `opened`/`reopened`/`ready_for_review` 时会由 `.github/workflows/claude-pr-review.yml` 自动跑一次 Claude review（判断标准见 `pr-review-checklist` skill），只发顶层评论，不提交正式 review 状态。
