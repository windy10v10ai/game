...
这类文档只写**目的、设计、关键信息与决策**，不复述代码已经表达的内容（口径同「注释规约」）：调用链、常量名与取值、某个函数做了什么，都从代码读，抄进文档只会先过期。决策的**理由**要留下——那是代码里读不出来的部分。

**实现落地后要回头更新文档**：待定方案改成已定、废弃的决策删掉、新踩的坑补上。这是完成一次改动的一部分，与改代码同一个 PR，不要等用户催。
