...
以下两种情况，Claude **必须主动提案**更新相关文档（CLAUDE.md 或对应 skill 文件），不需要等用户要求：

1. **用户纠正了 Claude 的做法**：若该错误根源是 CLAUDE.md 或 skill 文件的记载缺失、错误或与实际代码矛盾，立即指出"是哪条记载有问题"并提案修复。
2. **Claude 执行中发现文档与实际矛盾**：例如 CLAUDE.md 描述的路径不存在、skill 文件引用的字段已改名、记载的规则与代码实现不一致等，立即告知用户并提案更新。

3. **用户人工干预补充了 Claude 不知道的信息**：例如用户说明了某个约定、解释了某个路径规则、提供了 Claude 无法自行推断的上下文，此后该信息若对未来会话有价值，立即提案将其写入 CLAUDE.md 或对应 skill 文件。

提案格式：用 `AskUserQuestion` 工具以选项菜单形式向用户确认（选项至少含"确认写入"与"不写入"），并在问题描述中说明"哪里错了/缺了什么/新增了什么"及拟写入的内容；用户确认后再写入文件。
