...
function _ScoreboardUpdater_UpdatePlayerPanel(
  ...
  if (playerInfo) {
    ...
    playerPanel.SetHasClass(
      'local_player_teammate',
      isTeammate && playerId != Game.GetLocalPlayerID(),
    );

    _ScoreboardUpdater_SetTextSafe(
      playerPanel,
      'RespawnTimer',
      playerInfo.player_respawn_seconds + 1,
    ); // value is rounded down so just add one for rounded-up
      ...
      if (playerInfo.player_selected_hero !== '') {
        ...
        if (heroImageCustom && heroImageCustom !== '') {
          ...
          playerPortrait.SetImage(
            'file://{images}/heroes/' + playerInfo.player_selected_hero + '.png',
          );
        }
      } else {
  ...
  if (playerItemsContainer) {
    ...
    if (playerItems) {
      ...
      for (var i = playerItems.inventory_slot_min; i < playerItems.inventory_slot_max; ++i) {
        ...
        if (itemInfo) {
          var item_image_name =
            'file://{images}/items/' + itemInfo.item_name.replace('item_', '') + '.png';
...
//=============================================================================
//=============================================================================
function _ScoreboardUpdater_UpdateTeamPanel(
  scoreboardConfig,
  containerPanel,
  teamDetails,
  teamsInfo,
  ...
  if (GameUI.CustomUIConfig().team_colors) {
    ...
    var teamColor_GradentFromTransparentLeft = teamPanel.FindChildInLayoutFile(
      'TeamColor_GradentFromTransparentLeft',
    );
    if (teamColor_GradentFromTransparentLeft) {
      var gradientText =
        'gradient( linear, 0% 0%, 800% 0%, from( #00000000 ), to( ' + teamColor + ' ) );';
      //			$.Msg( gradientText );
...
function _ScoreboardUpdater_UpdateAllTeamsAndPlayers(scoreboardConfig, teamsContainer) {
  ...
  for (var i = 0; i < teamsList.length; ++i) {
    var teamPanel = _ScoreboardUpdater_UpdateTeamPanel(
      scoreboardConfig,
      teamsContainer,
      teamsList[i],
      teamsInfo,
...
function ScoreboardUpdater_SetScoreboardActive(scoreboardHandle, isActive) {
  ...
  if (isActive) {
    _ScoreboardUpdater_UpdateAllTeamsAndPlayers(
      scoreboardHandle.scoreboardConfig,
      scoreboardHandle.scoreboardPanel,
    );
  }
}
